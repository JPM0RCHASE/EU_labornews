...
# JP's Weekly Insight 주제 순환 (주차 기반 — 매주 다른 실무 주제)
_INSIGHT_TOPICS = [
    "임금·수당 실무 (통상임금 범위, 연장근로수당, 주휴수당, 퇴직금 계산 방법)",
    "해고·징계 절차 (부당해고 요건, 해고예고, 징계위원회 운영, 권고사직)",
    "근로시간·휴가 관리 (연차 산정, 대체휴무, 탄력근로제, 포괄임금제 유효성)",
    "산업재해·중대재해 대응 (업무상 재해 인정기준, 중대재해처벌법 의무)",
    "근로계약·취업규칙 (기간제 갱신기대권, 파견·도급 구분, 취업규칙 불이익변경)",
    "직장내 괴롭힘·성희롱 (신고·조사 절차, 사업주 의무, 비밀보호)",
    "4대보험·실업급여 (가입 의무, 실업급여 수급요건, 건강보험료 정산)",
    "외국인·특수고용·플랫폼 노동 (고용허가제, 근로자성 판단, 플랫폼 종사자 보호)",
]
_week_num = TODAY.isocalendar()[1]
_insight_topic = _INSIGHT_TOPICS[_week_num % len(_INSIGHT_TOPICS)]
...
【생성 규칙 — 섹션 선별 순서 반드시 준수】
※ 아래 순서대로 기사를 배정하면 자동으로 모든 섹션 중복이 방지됩니다.

STEP 1 — section5_ruling 먼저 확정
  - 판결 뉴스 목록에서 가장 중요한 판결 1건 선정
  - 판결 뉴스가 없으면 일반 뉴스 중 "판결·결정·선고" 키워드가 포함된 기사 1건 사용
  - 이 단계에서 선정한 기사·주제는 다른 섹션에서 절대 재사용 금지

STEP 2 — section2_gov_policy 확정
  - 고용노동부·국회·정부 정책 관련 기사 선정
  - STEP 1에서 이미 사용한 기사·주제와 겹치면 안 됨

STEP 3 — section1_top3 확정
  - STEP 1, STEP 2에서 사용한 기사·주제를 모두 제외하고 3건 선정
  - ★메이저 언론사(조선·중앙·동아·한겨레·경향·KBS·MBC·SBS·JTBC·연합뉴스) 우선
  - 삼성·SK·현대차·LG 등 대기업 노사·파업 뉴스 제외 (카드뉴스와 중복 방지)

STEP 4 — section4_five_fewer 확정
  - STEP 1~3에서 사용한 기사·주제와 겹치지 않는 5인 미만 관련 내용 선정
  - 해당 뉴스 없으면 공인노무사 JP 실무 인사이트로 대체

STEP 5 — section3_weekly_insight 확정 (질문 주제 지정)
  이번 주 Insight 주제: 【{_insight_topic}】
  - STEP 1~4 내용과 완전히 다른 독립적 실무 질문 1개 작성
  - HR담당자·소규모 사업주가 실제로 자주 묻는 구체적 질문 (예: "연차를 다 못 쓰면 수당으로 줘야 하나요?")
  - section1~5에서 다룬 뉴스 주제를 질문으로 반복하는 것 절대 금지

【공통 규칙】
1. 반드시 수집된 뉴스 목록에서만 선별할 것 (임의 생성 금지)
2. 없는 섹션은 공인노무사 JP 실무 인사이트로 대체 (url → https://laborjp.tistory.com)
3. 모든 제목은 질문형 또는 실용적 표현 권장
4. 동일 기사·동일 주제가 두 섹션 이상에 등장하면 생성 실패로 간주 — 반드시 각 섹션 고유 주제로 작성
