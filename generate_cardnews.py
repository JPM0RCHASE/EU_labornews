...
3번 — 직장 속 사건·사고 (이런 일 실제로...)
  ...
  · 독자가 "우리 회사에서도 생길 수 있다"고 느낄 수 있는 스토리 필수
  · 해당 없으면 → 직장내 괴롭힘 인정 기준 사례로 대체
...
def publish_and_wait(timeout: int = 300) -> bool:
    ...
    if git("diff", "--staged", "--quiet").returncode != 0:
        git("commit", "-m", f"Auto: {DATE_STR} 텔레그램 카드뉴스 자동 생성")
        git("pull", "--rebase", "--autostash", "origin", "main")
        push = git("push", "origin", "HEAD:main")
        if push.returncode != 0:
            print(f"⚠ 푸시 실패 — 링크가 바로 안 열릴 수 있음: {push.stderr.strip()[:300]}")
            return False
        print("✅ 카드뉴스 HTML 푸시 완료 — Vercel 배포 대기")
