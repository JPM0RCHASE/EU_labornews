...
# ── Naver 뉴스 수집 ──────────────────────────────────
KEYWORDS = [
    "노동조합 파업","임금체불 단속","부당해고 판결",
    "중대재해 처벌","고용노동부 정책","최저임금 심의",
    "5인미만 사업장 노동법","직장내괴롭힘","기간제 근로자",
    "HR 채용 트렌드","산업재해","고용보험","육아휴직",
]
...
아래 수집된 뉴스에서 경영자·사장님·HR 담당자에게 중요한 뉴스를 선별하여 블로그용 깊이있는 카드뉴스 8건을 작성하세요.

수집된 뉴스:
{news_text}

【필수 섹션 구조 - 반드시 준수】
섹션1 "노사 핫이슈": 2건 (파업, 노조, 원청교섭, 노사갈등)
섹션2 "판례·단속": 2건 (부당해고, 직장갑질, 산재, 임금체불 단속)
섹션3 "사장님 체크포인트": 2건 (인건비, 고용지원금, 정책변화, 규정준수)
섹션4 "5인 미만 사업장 필독": 1건 ⭐반드시 포함 (소상공인·자영업자 노동법 이슈)
섹션5 "HR 동향": 1건 (채용, 임금, AI, 조직문화, 교육)

【작성 기준】
- 뉴스가 부족한 섹션은 공인노무사 JP의 실무 인사이트로 채울 것 (is_insight_card: true)
- 블로그용이므로 불릿 포인트를 5개 이상 상세하게 작성
- 실무 시사점은 구체적이고 실행 가능한 내용으로 작성
- 5인 미만 사업장 섹션은 반드시 실제 현장에서 자주 발생하는 이슈로 작성
